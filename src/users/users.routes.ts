...
userRouter.get("/users/search", async (req: Request, res: Response) => {
    try {
        ...
        if (!name && !email) {
            return res.status(StatusCodes.BAD_REQUEST).json({ error: "Please provide a name or email for searching." });
        }
        ...
        if (foundUsers.length === 0) {
            return res.status(StatusCodes.NOT_FOUND).json({ msg: "No users found matching the criteria." });
        }
